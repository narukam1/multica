...
// Latin UI / mono / editorial stacks live in ./globals.css as `--font-inter`,
// `--font-mono`, and `--font-serif`. next/font/google is not used: `next build`
// fetches those faces from fonts.google.com, which isolated self-host builders
// cannot reach. CJK still uses the per-locale fallback chain in globals.css
// (must stay overridable per `<html lang>`). Desktop keeps its own
// `@fontsource-variable/*` files in apps/desktop.
